...
class member_orderControl extends BaseMemberControl {
    ...
    private function _order_receive($order_info, $post) {
        if (!chksubmit()) {
            ...
            $goods_info = $goodsModel->where(array('goods_id'=>$order_goods_info['goods_id']))->find();
            ...
            $rebate = $goods_common_info['rebate'];
            //添加到该用户的
            $memberModel = Model('member');
            $member_info = $memberModel->where(array('member_id'=>$order_info['buyer_id']))->find();
            //如果有上级用户
            if($member_info['inviter_id']){
                $drinksModel = Model('member');
                $sql = 'UPDATE 33hao_member set available_predeposit=available_predeposit+'.$rebate.' where member_id='.$member_info['inviter_id'];
                $res = $drinksModel->execute($sql);
                //dump($sql);
                //dump($res);
            }
            //dump($member_info['inviter_id']);
            //dump($rebate);die;
            //确认收货返利
