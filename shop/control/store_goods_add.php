...
class store_goods_addControl extends BaseSellerControl {
    ...
    public function save_goodsOp() {
        if (chksubmit()) {
            ...
            $common_array['goods_type']         = $_POST['goods_type'];
            $common_array['rebate']             = $_POST['rebate'];
            ...
            $common_array['spec_name']          = is_array($_POST['spec']) ? addslashes(serialize($_POST['sp_name'])) : serialize(null);
            $common_array['spec_value']         = is_array($_POST['spec']) ? addslashes(serialize($_POST['sp_val'])) : serialize(null);
 		...
 		require_once(BASE_RESOURCE_PATH.DS.'phpqrcode'.DS.'index.php');
                ...
                if (is_array($_POST['spec'])) {
                    foreach ($_POST['spec'] as $value) {
                        ...
                        $goods['goods_spec']        = addslashes(serialize($value['sp_value']));
                        ...
                        $id = $model_type->addGoodsType($goods_id, $common_id, array('cate_id' => $_POST['cate_id'], 'type_id' => $_POST['type_id'], 'attr' => $_POST['attr']));
			 ...
			 // 生成商品二维码
                    ...
                    $goods['goods_jingle']      = $common_array['goods_jingle'];
                    $goods['rebate']            = $common_array['rebate'];
                    $goods['store_id']          = $common_array['store_id'];
                    $goods['store_name']        = $_SESSION['store_name'];
                    $goods['gc_id']             = $common_array['gc_id'];
                    $goods['gc_id_1']           = $common_array['gc_id_1'];
