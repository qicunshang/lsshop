...
<div class="item-publish">
  <form method="post" id="goods_form" action="<?php if ($output['edit_goods_sign']) { echo urlShop('store_goods_online', 'edit_save_goods');} else { echo urlShop('store_goods_add', 'save_goods');}?>">
    ...
    <div class="ncsc-form-goods">
      ...
      <dl>
        <dt><i class="required">*</i>返利金额<?php echo $lang['nc_colon'];?></dt>
        <dd>
          <input name="rebate" type="text" class="text w400" value="<?php echo $output['goods']['rebate']; ?>" />
          <span></span>
      ...
      <dl nc_type="spec_group_dl_<?php echo $i;?>" nctype="spec_group_dl" class="spec-bg" <?php if($k == '1'){?>spec_img="t"<?php }?>>
        <dt>
          <input name="sp_name[<?php echo $k;?>]" type="text" class="text w60 tip2 tr" title="规格名称,为避免误操作,请不要随意修改" value="<?php if (isset($output['goods']['spec_name'][$k])) { echo $output['goods']['spec_name'][$k];} else {echo $val['sp_name'];}?>" maxlength="4" nctype="spec_name" data-param="{id:<?php echo $k;?>,name:'<?php echo $val['sp_name'];?>'}"/>
        ...
        <dd <?php if($k == '1'){?>nctype="sp_group_val"<?php }?>>
          <ul class="spec">
            ...
            <li data-param="{gc_id:<?php echo $output['goods_class']['gc_id'];?>,sp_id:<?php echo $k;?>,url:'<?php echo urlShop('store_goods_add', 'ajax_add_spec');?>'}">
              ...
              <div class="ncsc-btn">请勾选然后修改</div>
              <div nctype="specAdd2" style="display:none;">
                <input class="text w60" type="text" placeholder="规格值名称" style="">
...
$(function(){
	...
	$('dl[nctype="spec_group_dl"]').find('input[type="checkbox"]').each(function(){
		...
		if (!(typeof(E_SP[s]) == 'undefined')){
			...
			if(E_SP[s] != ''){
				$(this).val(E_SP[s]);
				v.html('<input type="text" maxlength="50" value="'+E_SP[s]+'" />');
			}else{
				v.html('<input type="text" maxlength="50" value="'+v.html()+'" />');
			}
